...
// Fairchild Channel-F, VMU, Atari 2600, Sharp MZ-700, Pokemon Mini, Bally
// Astrocade, Microvision, Game Pocket Computer Emulation Second Page for
// settings .mbap plugins pinned directories
...
void Show_help_window() {
  if (gl_select_lang == 0xE1E1) { // english
    DrawPic((u16 *)gImage_English_manual, 240 - 73, 160 - 73, 70, 70, 0, 0,
            1); //
  } else {
    DrawPic((u16 *)gImage_Chinese_manual, 240 - 73, 160 - 73, 70, 70, 0, 0,
            1); //
  }
...
void Show_ICON_filename(u32 show_offset, u32 file_select, u32 haveThumbnail) {
  ...
  for (line = 0; line < need_show_folder; line++) {
    ...
    if (line == file_select) {
      Clear(18, 20 + file_select * 14,
            ((char_num == 17) ? (17 * 6 + 1) : (240 - 17)), 13,
            gl_color_selectBG_sd, 1);
    }
    DrawPic((u16 *)(gImage_icons + 0 * 16 * 14 * 2), 2, y_offset + line * 14,
            16, 14, 1, 0x0000, 1);
    DrawHZText12(pFolder[show_offset + line].filename, char_num, 3 + 16,
  ...
  for (line = need_show_folder; line < need_show_folder + need_show_game;
    ...
    if (!strcasecmp(&(pfilename[strlen8 - 3]), "gba")) { // GBA
    ...
    } else if (!strcasecmp(&(pfilename[strlen8 - 2]),
                           "mb")) { // PogoShell Plugin/Multiboot image
      icon = (u16 *)(gImage_icon_EXE);
    } else if (!strcasecmp(&(pfilename[strlen8 - 3]),
                           "mbz")) { // Compressed PogoShell Plugin
      icon = (u16 *)(gImage_icon_EXE);
    } else if (!strcasecmp(&(pfilename[strlen8 - 4]),
                           "mbap")) { // Compressed PogoShell Plugin
      icon = (u16 *)(gImage_icon_EXE);
    ...
    } else if (!strcasecmp(&(pfilename[strlen8 - 3]), "ngp")) { // Neo Geo
                                                                // Pocket
      ...
      icon = (u16 *)(gImage_icon_TXT);
                           ...
                           "esv")) { // Fixes the bug with esv files looking
                                     // like watara supervision
      ...
      icon = (u16 *)(gImage_icon_PCE);
                           ...
                           "z80")) { // Sinclair ZX-Spectrum (Z80)
      icon = (u16 *)(gImage_icon_ZX);
    } else if (!strcasecmp(
                   &(pfilename[strlen8 - 2]),
                   "o2")) { // Magnavox Odyssey2 (No emu yet, but I'm eventually
                            // going to have a finished one. :D)
      ...
      icon = (u16 *)(gImage_icon_chip);
                           ...
                           "min")) { // Pokemon Mini (No Emu yet)
      icon = (u16 *)(gImage_icon_pokem);
    } else if (!strcasecmp(&(pfilename[strlen8 - 3]),
                           "dci")) { // Visual Memory Unit (No Emu yet, but I
                                     // will make one at some point)
      icon = (u16 *)(gImage_icon_vmu);
    } else if (!strcasecmp(&(pfilename[strlen8 - 3]),
                           "vmi")) { // Visual Memory Unit (No Emu yet, but I
                                     // will make one at some point)
      icon = (u16 *)(gImage_icon_vmu);
    ...
    } else if (!strcasecmp(&(pfilename[strlen8 - 3]), "nsf")) { // NSF sound
                                                                // file
      icon = (u16 *)(gImage_icon_mod);
    } else if (!strcasecmp(&(pfilename[strlen8 - 3]),
                           "k3m")) { // Krawall Advance Module
      icon = (u16 *)(gImage_icon_mod);
    } else if (!strcasecmp(&(pfilename[strlen8 - 3]),
                           "mod")) { // Protracker mod file
      icon = (u16 *)(gImage_icon_mod);
    } else if (!strcasecmp(&(pfilename[strlen8 - 3]),
                           "pcx")) { // ZSoft Paintbrush PCX image
      icon = (u16 *)(gImage_icon_IMG);
    } else if (!strcasecmp(&(pfilename[strlen8 - 3]), "vgm")) { // SMS/GG VGM
                                                                // Rip
      icon = (u16 *)(gImage_icon_mod);
    } else if (!strcasecmp(&(pfilename[strlen8 - 3]),
                           "cwz")) { // Unknown Music file, contained in a
                                     // package for PogoShell 1.2
      icon = (u16 *)(gImage_icon_mod);
    } else if (!strcasecmp(&(pfilename[strlen8 - 2]),
                           "sb")) { // MaxMod SoundBank
      icon = (u16 *)(gImage_icon_mod);
      ...
      icon = (u16 *)(gImage_icon_IMG);
                           ...
                           "lz")) { // LZ77 Compressed Mode 3 Bitmap
      icon = (u16 *)(gImage_icon_IMG);
    } else if (!strcasecmp(&(pfilename[strlen8 - 3]),
                           "bgf")) { // BoyScout module
      icon = (u16 *)(gImage_icon_mod);
    } else if (!strcasecmp(&(pfilename[strlen8 - 3]),
                           "arc")) { // 4kb Arcadia 2001 ROM File
      icon = (u16 *)(gImage_icon_arc);
    } else if (!strcasecmp(&(pfilename[strlen8 - 3]),
                           "a26")) { // Atari 2600 ROM file (emu indev)
      icon = (u16 *)(gImage_icon_a26);
    } else if (!strcasecmp(&(pfilename[strlen8 - 2]),
                           "sc")) { // Sega SC-3000 ROM File
    ...
    DrawPic(icon, 1, showy, 16, 14, 1, 0x0000, 1);
    DrawHZText12(pFilename_buffer[offset + line - need_show_folder].filename,
                 char_num, 3 + 16, showy, name_color, 1);
    if ((haveThumbnail == 1) && (line > 3)) {
    } else {
      char msg[20];
...
void IWRAM_CODE Refresh_filename(u32 show_offset, u32 file_select, u32 updown,
  ...
  if ((file_select == (need_show_folder - 1)) && (updown == 3)) {
    DrawHZText12(pFolder[show_offset + xx1].filename, char_num1, 3 + 16, showy1,
                 name_color1, 1);
    DrawHZText12(pFilename_buffer[0].filename, char_num2, 3 + 16, showy2,
                 name_color1, 1);
    if (char_num1 == 32) {
      sprintf(msg, "%s", "");
    ...
    DrawHZText12(pFolder[show_offset + xx1].filename, char_num1, 3 + 16, showy1,
                 name_color1, 1);
    DrawHZText12(pFolder[show_offset + xx2].filename, char_num2, 3 + 16, showy2,
                 name_color1, 1);
    sprintf(msg, "%s", "");
    if (char_num1 == 32) {
      DrawHZText12(msg, 0, 221, showy1, name_color1, 1);
    ...
    DrawHZText12(pFilename_buffer[offset + xx1 - need_show_folder].filename,
                 char_num1, 3 + 16, showy1, name_color1, 1);
    DrawHZText12(pFilename_buffer[offset + xx2 - need_show_folder].filename,
                 char_num2, 3 + 16, showy2, name_color1, 1);
    if (char_num1 == 32) {
...
void Show_ICON_filename_NOR(u32 show_offset, u32 file_select) {
  ...
  for (line = 0; line < need_show; line++) {
    ...
    DrawPic((u16 *)gImage_nor_icon /*(gImage_icons+2*16*14*2)*/, 2,
            y_offset + line * 14, 16, 14, 1, 0x0000, 1);
    DrawHZText12(pNorFS[show_offset + line].filename, char_num, 3 + 16,
                 y_offset + line * 14, name_color, 1);
...
void Refresh_filename_NOR(u32 show_offset, u32 file_select, u32 updown) {
  ...
  DrawHZText12(pNorFS[show_offset + xx1].filename, char_num, 3 + 16, showy1,
               name_color1, 1);
  DrawHZText12(pNorFS[show_offset + xx2].filename, char_num, 3 + 16, showy2,
               name_color1, 1);
...
void Filename_loop(u32 shift, u32 show_offset, u32 file_select,
  ...
  if (shift > timeout) {
    ...
    if (file_select < need_show_folder) {
      ...
      strncpy(
          temp_filename,
          pFilename_buffer[offset + file_select - need_show_folder].filename,
          100);
    }
    ...
    if (namelen > (char_num - 1)) {
      ...
      if (orgtt != tt) {
        ...
        Clear(18, 20 + file_select * 14, ((char_num)*6), 13,
              gl_color_selectBG_sd, 1);
        DrawHZText12(msg, char_num - 1, 3 + 16, y_offset + file_select * 14,
                     gl_color_text, 1);
      }
    }
  }
...
void Show_MENU(u32 menu_select, PAGE_NUM page, u32 havecht, u32 Save_num,
  ...
  for (line = 0; line < linemax; line++) {
    ...
    if (page == NOR_list) {
      DrawHZText12(gl_nor_op[line], 32, 47, y_offset + line * 14, name_color,
                   1);
    } else {
      if (line == 5) { // cheat
        ...
        DrawHZText12(gl_rom_menu[line], 32, 47, y_offset + line * 14,
                     name_color, 1);
        if (line == 4) { // save tpye
          switch (Save_num) {
          case 1:
          ...
          // ClearWithBG((u16*)gImage_MENU -64,60+60, y_offset + line*14, 10*6,
          // 13, 1);
...
void Show_Extra_Menu(u32 menu_select) {
  ...
  for (line = 0; line < linemax; line++) {
    ...
    DrawHZText12(gl_more_options[line], 32, 47, y_offset + line * 14,
                 name_color, 1);
  }
}
...
u32 show_recently_play(void) {
  ...
  DrawHZText12(gl_recently_play, 0, (240 - strlen(gl_recently_play) * 6) / 2, 4,
               rcolor, 1); // TITLE
  all_count = get_count();
  if (all_count) {
    setRepeat(15, 1);
...
#ifdef DEBUG
  // DEBUG_printf("first clust %x;  sec=%x ",(&file)->obj.sclust,
  // ClustToSect(&EZcardFs,(&file)->obj.sclust)	); DEBUG_printf("fs->fs_type
  // %x",(&EZcardFs)->fs_type);
#endif
...
u8 Check_saveMODE(u8 gamecode[]) {
  ...
  for (i = 0; i < 3000; i++) {
    if (memcmp(((SAVE_MODE_ *)pReadCache)[i].gamecode, "FFFF", 4) == 0) {
      break;
    } else if (memcmp(((SAVE_MODE_ *)pReadCache)[i].gamecode, gamecode, 4) ==
               0) {
...
u32 IWRAM_CODE Loadfile2PSRAM(TCHAR *filename) {
  ...
  if (res == FR_OK) {
    ...
    for (blocknum = 0x0000; blocknum < filesize; blocknum += 0x20000) {
      ...
      if ((gl_reset_on == 1) || (gl_rts_on == 1) || (gl_sleep_on == 1) ||
          (gl_cheat_on == 1)) {
...
void IWRAM_CODE make_pogoshell_arguments(TCHAR *cmdname, TCHAR *filename,
                                         u32 cmdsize, u32 filesize, u32 Address,
                                         u32 offset) {
  u32 *p, addr;
...
u32 IWRAM_CODE LoadEMU2PSRAM(TCHAR *filename, u32 is_EMU) {
  ...
  if (res == FR_OK) {
    ...
    for (blocknum = 0x0000; blocknum < filesize; blocknum += 0x20000) {
      ...
      SetPSRampage(page);
      dmaCopy((void *)pReadCache, PSRAMBase_S98 + rom_start_address + Address,
              0x20000);
      page = 0;
    }
    f_close(&gfile);
    ...
    if ((is_EMU > 3) && (is_EMU < 9)) {
      ...
      SetPSRampage(page);
      make_pogoshell_arguments(plugin + 9, filename, rom_start_address,
                               filesize, Address, offset);
    }
    SetPSRampage(0);
  ...
  if (folder_total > 1) {
    for (ret = 0; ret < folder_total - 1; ret++) {
      for (i = 0; i < folder_total - ret - 1; i++) {
        ...
        if (get > 0) {
          dmaCopy(&pFolder[i + 1], &(pFilename_temp.filename),
                  sizeof(FM_Folder_FS));
          dmaCopy(&pFolder[i], &pFolder[i + 1], sizeof(FM_Folder_FS));
          dmaCopy(&(pFilename_temp.filename), &pFolder[i],
                  sizeof(FM_Folder_FS));
        }
      }
    }
  ...
  if (game_total_SD > 1) {
    for (ret = 0; ret < game_total_SD - 1; ret++) {
      for (i = 0; i < game_total_SD - ret - 1; i++) {
        get = strcmp(pFilename_buffer[i].filename,
                     pFilename_buffer[i + 1].filename);
        if (get > 0) {
          dmaCopy(&pFilename_buffer[i + 1], &pFilename_temp,
                  sizeof(FM_FILE_FS));
          dmaCopy(&pFilename_buffer[i], &pFilename_buffer[i + 1],
                  sizeof(FM_FILE_FS));
...
u32 Load_Thumbnail(TCHAR *pfilename_pic) {
  ...
  if (res == FR_OK) {
    ...
    sprintf(picpath, "/SYSTEM/IMGS/%c/%c/%c%c%c%c.bmp", GAMECODE[0],
            GAMECODE[1], GAMECODE[0], GAMECODE[1], GAMECODE[2], GAMECODE[3]);
...
void SD_list_L_START(show_offset, file_select, folder_total) {
  ...
  Show_MENU_btn();
  DrawHZText12(gl_LSTART_help, 0, 60, 60, gl_color_text,
               1); // use sure?gl_LSTART_help
  DrawHZText12(
      pFilename_buffer[show_offset + file_select - folder_total].filename, 20,
      60, 75, 0x001F, 1);                          // file name
  ...
  while (1) {
    ...
    if (keysdown & KEY_A) {
      TCHAR *pdelfilename;
      pdelfilename =
          pFilename_buffer[show_offset + file_select - folder_total].filename;
...
void SD_list_L_SELECT(show_offset, file_select, folder_total) {
  ...
  DrawPic((u16 *)gImage_MENU, 36, 25, 168, 110, 1, 0, 1); // show menu pic
  Show_MENU_btn();
  DrawHZText12(gl_LSELECT_help, 0, 60, 60, gl_color_text,
               1); // use sure?gl_LSTART_help
  DrawHZText12(
      pFilename_buffer[show_offset + file_select - folder_total].filename, 20,
      60, 75, 0x001F, 1);                          // file name
...
void Backup_savefile(const char *filename) {
  ...
  strncpy(temp_filename + temp_filename_length, filename,
          sizeof(temp_filename) - temp_filename_length - 2);
  temp_filename_length = strlen(temp_filename);
...
int main(void) {
  ...
  if ((Current_FW_ver < Built_in_ver) ||
      (Current_FW_ver == 99)) { // 99 is test ver
  ...
  */
  ...
  game_total_NOR = GetFileListFromNor(); // initialize to prevent direct writes
                                         // to NOR without page turning
  if (game_total_NOR == 0) {
  ...
  if (page_num == SD_list) {
    ...
    if (res == FR_OK) {
      while (1) {
        ...
        // DEBUG_printf("=%x %s %x %x",res,
        // fileinfo.fname,fileinfo.fname[0],fileinfo.fattrib); wait_btn();
        if (res != FR_OK || fileinfo.fname[0] == 0) {
          break;
        }
        if ((fileinfo.fattrib == AM_DIR) ||
            (fileinfo.fattrib == 0x30)) { // DIR and exFAT dir
...
re_showfile:
  ...
  while (1) {
    while (1) { // 2
      ...
      if (updata && gl_show_Thumbnail) {
        ...
        if (page_num == SD_list) {
          pfilename_pic =
              pFilename_buffer[show_offset + file_select - folder_total]
                  .filename;
        } else {
        ...
        if ((!strcasecmp(&(pfilename_pic[strlengba - 3]), "gba")) ||
            (!strcasecmp(&(pfilename_pic[strlengba - 3]), "bin")) ||
            (!strcasecmp(&(pfilename_pic[strlengba - 3]), "agb"))) {
          is_GBA = 1;
      ...
      if (updata == 1) { // reshow all
        if (page_num == SD_list) {
          ...
          Show_ICON_filename(show_offset, file_select,
                             gl_show_Thumbnail && is_GBA);
        ...
        if (page_num == NOR_list) {
          ...
          Refresh_filename(show_offset, file_select, updata,
                           gl_show_Thumbnail && is_GBA);
      ...
      if (updata && gl_show_Thumbnail && is_GBA && (page_num == SD_list)) {
        if (haveThumbnail) {
          DrawPic((u16 *)(pReadCache + 0x10036), 120, 80, 120, 80, 0, 0,
                  1); // show game pic
        } else {
          DrawPic((u16 *)(gImage_NOTFOUND), 120, 80, 120, 80, 0, 0,
                  1); // show game pic
        }
      ...
      if (keysrepeat & KEY_DOWN) {
        ...
        if (page_num == SD_list) {
          // res = f_getcwd(currentpath, sizeof currentpath / sizeof
          // *currentpath);
        ...
        /*
                if (key_L) {
                        if (show_offset + file_select >= folder_total) {
                                SD_list_L_SELECT(show_offset, file_select,
        folder_total); goto refind_file;
                        }
                }
        else {
                */
        DrawPic((u16 *)gImage_MENU, 36, 25, 168, 110, 1, 0,
                1); // show menu pic - (posx, posy, width, height)
        Show_MENU_btn();
        u8 MENU_line = 0;
        u8 re_menu = 1;
        u8 MENU_max = 3;
    ...
    if (is_EMU == 0xff) {
      ...
      MENU_max = (page_num == NOR_list)
                     ? 2
                     : (4 + ((gl_cheat_on == 1) ? ((havecht > 0) ? 1 : 0) : 0));
    }
  re_show_menu:
    DrawPic((u16 *)gImage_MENU, 36, 25, 168, 110, 1, 0, 1); // show menu pic
    Show_MENU_btn();
    while (1) { // 3
      if (re_menu) {
        Show_MENU(MENU_line, page_num, ((havecht > 0) ? 1 : 0), Save_num,
                  is_EMU);
      }
      VBlankIntrWait();
      re_menu = 0;
  ...
  load_file:
    ...
    // DrawHZText12(gl_Loading,0,(240-strlen(gl_Loading)*6)/2,74,
    // gl_color_text,1);
    u32 gamefilesize = 0;
    u32 savefilesize = 0;
    u32 ret;
    ...
    default:
      savefilesize = 0x10000;
      break; // UNKNOW,FF  for homebrew SRAM_TYPE	//2018-4-23 some emu
             // homebrew need 64kByte
    }
    ...
    if (MENU_line < 2) { // PSRAM DirectPSRAM or soft reset   or run NOR game
      ...
      FAT_table_buffer[0x1F8 / 4] =
          (&EZcardFs)->csize; // 0x40;  //sector of cluster
      FAT_table_buffer[0x1FC / 4] =
          (saveMODE << 24) | savefilesize; // save mode and save file size
      ...
      // FAT_table_buffer[0x200/4],FAT_table_buffer[0x204/4]); DEBUG_printf("
      // %08X %08X ", FAT_table_buffer[0x208/4],FAT_table_buffer[0x20C/4]);
      // DEBUG_printf(" %08X %08X ",
      // FAT_table_buffer[0x1F0/4],FAT_table_buffer[0x1F4/4]); DEBUG_printf("
      // %08X %08X ", FAT_table_buffer[0x1F8/4],FAT_table_buffer[0x1FC/4]);
    }
    if (is_EMU) {
      ShowbootProgress(gl_generating_emu);
      f_chdir(currentpath);                // return to game folder
    ...
    if (page_num == NOR_list) { // boot nor game
      ...
      Send_FATbuffer(FAT_table_buffer, 1); // only save FAT
      // wait_btn();
      SetRompageWithHardReset(pNorFS[show_offset + file_select].rompage,
                              gl_toggle_reset);
      while (1) {
        VBlankIntrWait();
      }
      ...
      case 2:                 // WRITE TO NOR CLEAN
        ...
        if (res == 0) {
          ...
          DrawHZText12("NOR Full!", 0, (240 - (9 * 6)) / 2, 86,
                       gl_color_NORFULL, 1);
          wait_btn();
          goto refind_file;
        }
